...
describe("mainTest", () => {
    ...
    it("Device Writable Properties Response", async function() {
        const payload = { "temprature" : (Math.floor(Math.random() * 10000000000).toString())};
        const writable_properties_cmd = new e2eTestCommands.CommandTestData_WritablePropertiesResponseCommands("temprature", payload["temprature"]);
        ...
        assert.ok(twinObject.reported.temprature);
        assert.ok(twinObject.reported.temprature.ac);
        assert.strictEqual(twinObject.reported.temprature.ac, 200);
        assert.ok(twinObject.reported.temprature.av);
        assert.ok(twinObject.reported.temprature.value);
        assert.strictEqual(twinObject.reported.temprature.value, payload["temprature"]);
    })

    it("Device reported property with component", async function() {
        const payload = { "temprature" : (Math.floor(Math.random() * 10000000000).toString())}
        const reported_properties_cmd = new e2eTestCommands.CommandTestData_ReportComponentPropertiesCommands("thermostate", "temprature", payload["temprature"]);
        let expectedMessage = await iothubTestCore.runTestCommand(testHubConnectionString, testDeviceInfo.deviceId, reported_properties_cmd);
        let rc = await iothubTestCore.verifyTelemetryMessage(testHubConnectionString, testDeviceInfo.deviceId, expectedMessage);
        let twin = await iothubTestCore.getTwinProperties(testHubConnectionString, testDeviceInfo.deviceId);
        console.log(twin)
        ...
        assert.ok(twinObject.reported.thermostate.temprature);
        assert.strictEqual(twinObject.reported.thermostate.temprature, payload["temprature"]);
    })
})
